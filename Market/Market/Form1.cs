...
namespace Market
{
    public partial class Form1 : Form
    {
        ...
        {
            // kurumsal müşterilerden yiyecek alan ilk kisinin varsa aldığı urunlerin 
            //toplamı.
            var ID = FakeDB.Satislar.FirstOrDefault(x=>x.AlanKisi.KullaniciTipi==
            KullaniciTipi.Kurumsal && x.SatilanUrun.UrunTuru==UrunTuru.Yiyecek);
            if (ID!=null)
            {
                var Toplam = FakeDB.Satislar.Where(x => x.AlanKisi.KullaniciAdi == ID.AlanKisi.KullaniciAdi)
                    .Sum(y => y.SatilanUrun.SatisFiyati);
                MessageBox.Show(Toplam.ToString());   
            }
            else
            {
                MessageBox.Show("Kurumsal firmalardan yiyecek alan yok.");
            }
        }
    }
}
